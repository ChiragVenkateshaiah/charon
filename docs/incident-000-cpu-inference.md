# Incident 000: Five Minutes for One Slack Reply
...
model stood in to test the bot's GenAI capability end-to-end. Replies took roughly five
minutes each. This document is the writeup of *why*, as best as it can be reconstructed
after the fact — and an honest account of where the reconstruction runs out.
...
| Input | Value | Status |
|---|---|---|
| Response time | ~5 minutes per message | **Measured** (observed repeatedly in production use) |
...
| Single- vs dual-channel memory (actual) | Not recorded | **Unknown** — TODO, recoverable if the laptop is still available |
| Exact model name/tag | Not recorded | **Unknown** — TODO, may be recoverable from Ollama's local model cache or shell history if the machine still has it |
...
At an expected 4–7 tokens/sec — call it 5 as a round number — five minutes of wall
time implies:

```
5 tokens/sec × 300 seconds = ~1,500 output tokens
```

That is far more text than a Slack bot reply would plausibly generate. Something in the
5-minute figure is not explained by decode bandwidth alone. This is the actual finding
of this incident writeup: **the bandwidth story is necessary but not sufficient.**
...
- **Single-channel memory.** If true, halves effective bandwidth (see above) but still
  doesn't obviously get to 1,500-token-equivalent wall time on its own.
- **Prefill cost on a long system prompt.** Prefill is compute-bound and scales with
  ...
  descriptions, or conversation history could spend a large, unmeasured chunk of the
  five minutes in prefill before the first output token even appears — none of the
...
This gap — a plausible mechanism (bandwidth) that explains roughly a third of the
...
## TODOs (recoverable, if I choose to chase them)

- [ ] Confirm exact model name/tag used at the time (Ollama local cache or shell
      history, if the machine is still available)
- [ ] Confirm actual memory configuration (single vs. dual channel, actual speed)
